...
lv, _junk, _junk, _junk, _junk, _junk = %x( mount|grep 'on / ').split(' ')
root_vg, root_lv = lv.sub('/dev/mapper/', '').split('-')

vgs_output = %x( vgs --nosuffix --units m ).split("\n")
...
lvs_output = %x( lvs  --nosuffix --units m ).split("\n")
...
pvs_output = %x( pvs  --nosuffix --units m ).split("\n")
